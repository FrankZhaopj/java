package com.zhaopj.java.excel.exppng;
 ...
 * 将指定excel的内容导出成图片格式
 * 参考博客
 * 	http://www.cnblogs.com/newflydd/p/4912662.html
 */
public class DrawFromExcel {
	
	public static void main(String[] args) throws Exception {
        // 给定两个初始值，标志出导出区域，两个行列组合的单元格
        int[] fromIndex = { 0, 0 };//起始单元格：列下标,行下标(都是从0开始)
        int[] toIndex = { 30, 21 };//结束单元格：列下标,行下标(都是从0开始)
        //int[] toIndex = { 7, 5 };//结束单元格：列下标,行下标(都是从0开始)
 
        int imageWidth = 0;
        int imageHeight = 0;
 
        Workbook wb = WorkbookFactory.create(new File("d:\\test2.xls"));
        ...
        imageHeight = imageHeight * 96 / 72 + 6;
        imageWidth = imageWidth * 115 / 100 + 7;
